...
\subsection{Punkt-Addition und -Verdopplung}
Die Punkt-Addition und Punkt-Verdopplung basieren auf den Formeln aus Abschnitt \ref{sec:ell}. \\
...
\begin{table} [h]
	...
	\begin{tabular}{ | p{3cm} | p{12cm} | }
		...
		\hline
		x2\_o & x Komponenten der Punkt-Dopplung \\
		\hline
		y2\_o & y Komponenten der Punkt-Dopplung \\
		\hline
		ready\_o & Status-Flag ob die Berechnung abgeschlossen ist  \\
		\hline
		\hline
	\end{tabular}
	\caption{Eingabe- und Ausgabeparameter der Punkt-Dopplungs-Entität}
...
\begin{table} [h]
	...
	\begin{tabular}{ | p{3cm} | p{12cm} | }
		...
		\hline
	\end{tabular}
	\caption{Eingabe- und Ausgabeparameter der Punkt-Additions-Entität}
	\label{tab:vhdl-impl-eccadd-param}
\end{table}

\subsection{Punkt-Multiplikation}
Die Punkt-Multiplikation basiert auf dem Double-And-Add-Algorithmus. Der Pseudocode ist in Listing \ref{lst-pm-algo} zu finden. \\
...
\begin{table} [h]
	...
	\begin{tabular}{ | p{3cm} | p{12cm} | }
		...
		\hline
	\end{tabular}
	\caption{Eingabe- und Ausgabeparameter der Punkt-Multiplikations-Entität}
...
\\ \\
Der Tabelle \ref{tab:vhdl-impl-testbenches} ist zu entnehmen, dass beispielsweise die Punkt-Multiplikation neben der eigentlichen Multiplikation auch die Punkt-Addition und damit auch die dafür notwendigen mathematischen Grundoperatoren verifiziert. Da es die Testbench erfordert, dass verschiedene Entitäten separat entwickelt werden, was im Fehlerfall die Indentifizierung deutlich erschwert, wurden weitere einfachere Testbenches geschrieben um zunächst die Basisfunktionalität der Entitäten zu verifizieren.
\\ \\
...
\begin{table} [h]
	...
	\begin{tabular}{ | p{5cm} | p{10cm} | }
		\hline
		\textbf{Beschreibung} & \textbf{Bedingung}\\
		\hline
		tb\_ecdsa & \parbox[t]{5cm}{$(r,s)=sign(m)$\\$verify((r,s),m)=True$}  \\
		\hline
		tb\_gf2m\_eea\_inversion & $x * x^-1 = 1$ \\
		\hline
		tb\_gf2m\_divider & \parbox[t]{5cm}{$c=a/b$\\$c*b=a$} \\
		\hline
		tb\_gf2m\_point\_multiplication & \parbox[t]{10cm}{$k.P = (k-1).P + P$\\$k.P = (n-1).P = -P = (xP, xP+yP)$} \\
		\hline
	\end{tabular}
